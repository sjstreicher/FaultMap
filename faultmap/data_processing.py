...
def process_aux_file(filename, bias_correct=True, mi_scale=False, allow_neg=False):
    ...
    # These are set when parsing the header row (row_index == 0)
    destination_var_index: int = 0
    max_val_index: int = 0
    bias_mean_index: int | None = None
    thresh_index: int = 0
    mi_index: int | None = None
    pass_threshold_index: int = 0
    pass_directionality_index: int = 0
    max_delay_index: int = 0
                    ...
                    if "sigtested" in dir_parts_inner:
                        ...
                        for path_part in dir_parts_inner[1:]:
                            nosigtest_save_dir_inner = Path(
                                nosigtest_save_dir_inner, path_part
                            )
                        ...
                        if boxindex > 0:
                            ...
                            nosigtest_final_weight_array_dir = Path(
                                nosigtest_save_dir_inner, weight_array_name, box
                            )
                        ...
                        nosigtest_difweightarray_dir = Path(
                            nosigtest_save_dir_inner, difweightarray_name, box
                        )
...
def create_signtested_directionalarrays(datadir, writeoutput):
    ...
    for test_index, test_string in enumerate(test_strings):
        if test_string in directories:
            ...
            else:  # test_string == "sigweight_directional_arrays"
...
def trend_extraction(mode: RunModes, case: str, write_output: bool) -> None:
...
def read_timestamps(raw_tsdata: str | Path) -> NDArray:
    ...
    return np.asarray(timestamp_list)
...
def writecsv(
    filename: str | Path,
    items: list | NDArray,
    header: list[str] | NDArray | None = None,
) -> None:
    """Write CSV directly"""
...
def detrend_linear_model(data: NDArray) -> NDArray:
    ...
    detrended_df = pd.DataFrame(signal.detrend(df.dropna().values, axis=0))
...
def normalise_data(
    ...
    scenario: str,
    method: str | bool,
    scalingvalues: dict | pd.DataFrame | None,
) -> NDArray:
...
def read_scale_limits(scaling_loc: str | Path):
...
def buildcase(dummyweight, digraph, name, dummycreation):
    ...
    connection = nx.to_numpy_array(digraph, weight=None)  # type: ignore[arg-type]
